...
class MC4WP_Subscribe_Request extends MC4WP_Request {
	...
	public function process() {
		...
		foreach ( $this->list_fields_map as $list_id => $list_field_data ) {
			...
			do_action( 'mc4wp_subscribe', $this->data['EMAIL'], $list_id, $list_merge_vars, $result, 'form', 'form', $this->form->ID );
		}
	...
	protected function send_email() {
		...
		<p><?php printf( __( '<strong>%s</strong> signed-up at %s on %s using the form "%s".', 'mailchimp-for-wp' ), $this->data['EMAIL'], date( 'H:i' ), date( 'd/m/Y' ), $this->form->name ); ?></p>
		 ...
		 */
		$receivers = apply_filters( 'mc4wp_email_summary_receiver', $to, $this->form->ID, $this->data['EMAIL'], $this->list_fields_map );
		 ...
		 */
		$subject = apply_filters( 'mc4wp_email_summary_subject', $subject, $this->form->ID, $this->data['EMAIL'], $this->list_fields_map );
		 ...
		 */
		$message = apply_filters( 'mc4wp_email_summary_message', $message, $this->form->ID, $this->data['EMAIL'], $this->list_fields_map );
